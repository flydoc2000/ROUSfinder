...
#      -k keep temp files
#      -c cutoff value for calling a repeat a large repeat instead of a ROUS (repeat of unusual size)
...
parser.add_argument('-c', action='store', dest='cutoff', help='Maximum size to call something a ROUS vs. a large repeat, default is 999', default='999')
...
for i in range(len(uniques)):
    ...
    if int(length)>cutoff:
        lr_count += 1
        g.write('>Large_repeat_'+str(lr_count)+'\n'+seq+'\n')
        repcopies.append(['Large_repeat_'+str(lr_count),length])
    else:
        rous_count += 1
        g.write('>ROUS_'+str(rous_count)+'\n'+seq+'\n')
        repcopies.append(['ROUS_'+str(rous_count),length])
...
rc = open(outcount,'w')
rc.write('Sequence\tGenome_size\tNumLR\tAvgSize\tAvgCopyNum\tNumROUS\tAvgSize\tAvgCopyNum\n')
...
if numlr == 0:
    ...
    avsizelr = sizelr/numlr
    avcopylr = copylr/numlr
...
rc.write(seqname+'\t'+str(seqlen)+'\t'+str(numlr)+'\t'+str(avsizelr)+'\t'+str(avcopylr)+'\t'+str(numrous)+'\t'+str(avsizerous)+'\t'+str(avcopyrous)+'\n')
